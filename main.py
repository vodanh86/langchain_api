...
def chat(query_input: QueryInput):
    ...
    query_type = "valid"
    #query_type = classify_query(query_input.question)
    if query_type == "nonsense":
        answer = "Xin lỗi, tôi không thể hiểu câu hỏi của bạn. Vui lòng đặt câu hỏi rõ ràng hơn."
        return QueryResponse(answer=answer, contexts="", session_id=session_id, model=query_input.model)
    elif query_type == "general":
        answer = "Đây là một câu hỏi chung chung. Vui lòng cung cấp thêm thông tin nếu bạn cần hỗ trợ cụ thể."
        return QueryResponse(answer=answer, contexts="", session_id=session_id, model=query_input.model)
...
def upload_and_index_document(file: UploadFile = File(...), dept_id: int = Form(...), upload_link: str = Form(...), effective_date: str = Form(...),):
    ...
    if file_extension not in allowed_extensions:
        raise HTTPException(
            status_code=400, detail=f"Unsupported file type. Allowed types are: {', '.join(allowed_extensions)}")
